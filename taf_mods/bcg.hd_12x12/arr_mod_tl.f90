...
integer, private, parameter :: ihop_max_nc_size = 102
integer, private, parameter :: ihop_max_range = 25
...
integer, private, parameter :: mpi_err_session = 75
...
integer, private, parameter :: nobsmax_ihop = 750
integer, private, parameter :: npx = 12
integer, private, parameter :: npy = 12
integer, private, parameter :: nr = 100
...
integer, private, parameter :: snx = 32
integer, private, parameter :: nx = snx*nsx*npx
integer, private, parameter :: sny = 37
...
type, private :: arrival_tl
  real(kind=8) :: srcdeclangle_tl
  real(kind=8) :: rcvrdeclangle_tl
  real(kind=8) :: a_tl
  real(kind=8) :: phase_tl
  complex(kind=8) :: delay_tl
  real(kind=8) :: delayr_tl
end type
...
type(arrival_tl), public, allocatable :: arr_tl(:,:,:)
...
contains
  subroutine addarr( afreq, iz, ir, amp, phase, delay, rcvrdeclangle, ntopbnc, nbotbnc )
!******************************************************************
!******************************************************************
!** This routine was generated by Automatic differentiation.     **
!** FastOpt: Transformation of Algorithm in Fortran, TAF 6.8.11  **
...
! declare arguments
!==============================================
  real(kind=8), intent(in) :: afreq
  real(kind=8), intent(in) :: amp
  complex(kind=8), intent(in) :: delay
  integer, intent(in) :: ir
  integer, intent(in) :: iz
  integer, intent(in) :: nbotbnc
  integer, intent(in) :: ntopbnc
...
!==============================================
  ...
  newray =  .true. 
  if (nt >= 1) then
    if (afreq*abs(delay-arr(nt,ir,iz)%delay) < phasetol .and. abs(arr(nt,ir,iz)%phase-phase) < phasetol) then
      newray =  .false. 
    endif
  endif
  if (newray) then
    if (nt >= nmaxarr) then
      iarr = minloc(arr(:,ir,iz)%a)
      if (amp > arr(iarr(1),ir,iz)%a) then
        arr(iarr(1),ir,iz)%a = amp
        arr(iarr(1),ir,iz)%phase = phase
        arr(iarr(1),ir,iz)%delay = delay
        arr(iarr(1),ir,iz)%delayr = real(delay)
        arr(iarr(1),ir,iz)%srcdeclangle = srcdeclangle
        arr(iarr(1),ir,iz)%rcvrdeclangle = rcvrdeclangle
        arr(iarr(1),ir,iz)%ntopbnc = ntopbnc
        arr(iarr(1),ir,iz)%nbotbnc = nbotbnc
      endif
    else
      nt = nt+1
      arr(nt,ir,iz)%a = amp
      arr(nt,ir,iz)%phase = phase
      arr(nt,ir,iz)%delay = delay
      arr(nt,ir,iz)%delayr = real(delay)
      arr(nt,ir,iz)%srcdeclangle = srcdeclangle
      arr(nt,ir,iz)%rcvrdeclangle = rcvrdeclangle
      arr(nt,ir,iz)%ntopbnc = ntopbnc
      arr(nt,ir,iz)%nbotbnc = nbotbnc
    endif
  else
    amptot = arr(nt,ir,iz)%a+amp
    w1 = arr(nt,ir,iz)%a/amptot
    w2 = amp/amptot
    arr(nt,ir,iz)%delay = w1*arr(nt,ir,iz)%delay+w2*delay
    arr(nt,ir,iz)%delayr = real(arr(nt,ir,iz)%delay)
    arr(nt,ir,iz)%a = amptot
    arr(nt,ir,iz)%srcdeclangle = w1*arr(nt,ir,iz)%srcdeclangle+w2*srcdeclangle
    arr(nt,ir,iz)%rcvrdeclangle = w1*arr(nt,ir,iz)%rcvrdeclangle+w2*rcvrdeclangle
  endif
  narrival(ir,iz) = nt
  end subroutine addarr
  subroutine addarr_tl( afreq, iz, ir, amp, amp_tl, phase, phase_tl, delay, delay_tl, rcvrdeclangle, rcvrdeclangle_tl, ntopbnc, nbotbnc )
!******************************************************************
!******************************************************************
!** This routine was generated by Automatic differentiation.     **
!** FastOpt: Transformation of Algorithm in Fortran, TAF 6.8.11  **
...
!==============================================
  ...
  complex(kind=8), intent(in) :: delay
  complex(kind=8), intent(in) :: delay_tl
  integer, intent(in) :: ir
  integer, intent(in) :: iz
  integer, intent(in) :: nbotbnc
  integer, intent(in) :: ntopbnc
  real(kind=8), intent(in) :: phase
  real(kind=8), intent(in) :: phase_tl
  real(kind=8), intent(in) :: rcvrdeclangle
  real(kind=8), intent(in) :: rcvrdeclangle_tl
...
!----------------------------------------------
  ...
  if (newray) then
    if (nt >= nmaxarr) then
      iarr = minloc(arr(:,ir,iz)%a)
      if (amp > arr(iarr(1),ir,iz)%a) then
        arr_tl(iarr(1),ir,iz)%a_tl = amp_tl
        arr(iarr(1),ir,iz)%a = amp
        arr_tl(iarr(1),ir,iz)%phase_tl = phase_tl
        arr(iarr(1),ir,iz)%phase = phase
        arr_tl(iarr(1),ir,iz)%delay_tl = delay_tl
        arr(iarr(1),ir,iz)%delay = delay
        arr_tl(iarr(1),ir,iz)%delayr_tl = real(delay_tl)
        arr(iarr(1),ir,iz)%delayr = real(delay)
        arr_tl(iarr(1),ir,iz)%srcdeclangle_tl = 0._ikind8
        arr(iarr(1),ir,iz)%srcdeclangle = srcdeclangle
        arr_tl(iarr(1),ir,iz)%rcvrdeclangle_tl = rcvrdeclangle_tl
        arr(iarr(1),ir,iz)%rcvrdeclangle = rcvrdeclangle
        arr(iarr(1),ir,iz)%ntopbnc = ntopbnc
        arr(iarr(1),ir,iz)%nbotbnc = nbotbnc
      endif
      ...
      nt = nt+1
      arr_tl(nt,ir,iz)%a_tl = amp_tl
      arr(nt,ir,iz)%a = amp
      arr_tl(nt,ir,iz)%phase_tl = phase_tl
      arr(nt,ir,iz)%phase = phase
      arr_tl(nt,ir,iz)%delay_tl = delay_tl
      arr(nt,ir,iz)%delay = delay
      arr_tl(nt,ir,iz)%delayr_tl = real(delay_tl)
      arr(nt,ir,iz)%delayr = real(delay)
      arr_tl(nt,ir,iz)%srcdeclangle_tl = 0._ikind7
      arr(nt,ir,iz)%srcdeclangle = srcdeclangle
      arr_tl(nt,ir,iz)%rcvrdeclangle_tl = rcvrdeclangle_tl
      arr(nt,ir,iz)%rcvrdeclangle = rcvrdeclangle
      arr(nt,ir,iz)%ntopbnc = ntopbnc
      arr(nt,ir,iz)%nbotbnc = nbotbnc
    endif
  else
    amptot_tl = amp_tl+arr_tl(nt,ir,iz)%a_tl
    amptot = arr(nt,ir,iz)%a+amp
    w1_tl = (-(amptot_tl*(arr(nt,ir,iz)%a/amptot/amptot)))+arr_tl(nt,ir,iz)%a_tl/amptot
    w1 = arr(nt,ir,iz)%a/amptot
    w2_tl = amp_tl/amptot-amptot_tl*(amp/amptot/amptot)
    w2 = amp/amptot
    arr_tl(nt,ir,iz)%delay_tl = arr_tl(nt,ir,iz)%delay_tl*w1+delay_tl*w2+w1_tl*arr(nt,ir,iz)%delay+w2_tl*delay
    arr(nt,ir,iz)%delay = w1*arr(nt,ir,iz)%delay+w2*delay
    arr_tl(nt,ir,iz)%delayr_tl = real(arr_tl(nt,ir,iz)%delay_tl)
    arr(nt,ir,iz)%delayr = real(arr(nt,ir,iz)%delay)
    arr_tl(nt,ir,iz)%a_tl = amptot_tl
    arr(nt,ir,iz)%a = amptot
    arr_tl(nt,ir,iz)%srcdeclangle_tl = arr_tl(nt,ir,iz)%srcdeclangle_tl*w1+w1_tl*arr(nt,ir,iz)%srcdeclangle+w2_tl*srcdeclangle
    arr(nt,ir,iz)%srcdeclangle = w1*arr(nt,ir,iz)%srcdeclangle+w2*srcdeclangle
    arr_tl(nt,ir,iz)%rcvrdeclangle_tl = arr_tl(nt,ir,iz)%rcvrdeclangle_tl*w1+rcvrdeclangle_tl*w2+w1_tl*arr(nt,ir,iz)%rcvrdeclangle+w2_tl*rcvrdeclangle
    arr(nt,ir,iz)%rcvrdeclangle = w1*arr(nt,ir,iz)%rcvrdeclangle+w2*rcvrdeclangle
  endif
  narrival(ir,iz) = nt

  end subroutine addarr_tl
  subroutine arr_mod_constructor_tl()
!******************************************************************
!******************************************************************
!** This routine was generated by Automatic differentiation.     **
!** FastOpt: Transformation of Algorithm in Fortran, TAF 6.8.11  **
...
!----------------------------------------------
! ALLOCATE AND RESET MODULE VARIABLES PP
!----------------------------------------------
  if (allocated(arr)) then
    if ( .not. allocated(arr_tl)) then
      allocate( arr_tl(lbound(arr,1):ubound(arr,1),lbound(arr,2):ubound(arr,2),lbound(arr,3):ubound(arr,3)) )
      arr_tl(:,:,:) = arrival_tl(srcdeclangle_tl=0._ikind1,rcvrdeclangle_tl=0._ikind2,a_tl=0._ikind3,phase_tl=0._ikind4,delay_tl=(0._ikind5,&
&0._ikind5),delayr_tl=0._ikind6)
    endif
  endif

  end subroutine arr_mod_constructor_tl
  subroutine arr_mod_destructor_tl()
!******************************************************************
!******************************************************************
!** This routine was generated by Automatic differentiation.     **
!** FastOpt: Transformation of Algorithm in Fortran, TAF 6.8.11  **
...
!----------------------------------------------
  if (allocated(arr_tl)) then
    deallocate( arr_tl )
  endif

  end subroutine arr_mod_destructor_tl
...
!==============================================
  ...
  if (storage_size(singlearrival%phase) == 64) then
    mpi_rl = mpi_double_precision
    mpi_cl = mpi_double_complex
  else if (storage_size(singlearrival%phase) == 32) then
    mpi_rl = mpi_real
    mpi_cl = mpi_complex
  else
  ...
  end subroutine arrivaltypeinit
  subroutine arrivaltypeinit_tl( singlearrival, singlearrival_tl, nextarrival, nextarrival_tl )
!******************************************************************
!******************************************************************
!** This routine was generated by Automatic differentiation.     **
!** FastOpt: Transformation of Algorithm in Fortran, TAF 6.8.11  **
...
!==============================================
  type(arrival), intent(in) :: nextarrival
  type(arrival_tl), intent(in) :: nextarrival_tl
  type(arrival), intent(in) :: singlearrival
  type(arrival_tl), intent(in) :: singlearrival_tl
...
!----------------------------------------------
  ...
  if ( .not. arrival_type_committed_tl) then
    bl = 1
    call mpi_get_address( singlearrival_tl,base,ierr )
    n = 1
    call mpi_get_address( singlearrival_tl%a_tl,addr(n),ierr )
    ty(n) = mpi_rl
    n = n+1
    call mpi_get_address( singlearrival_tl%phase_tl,addr(n),ierr )
    ty(n) = mpi_rl
    n = n+1
    call mpi_get_address( singlearrival_tl%delay_tl,addr(n),ierr )
    ty(n) = mpi_cl
    n = n+1
    call mpi_get_address( singlearrival_tl%delayr_tl,addr(n),ierr )
    ty(n) = mpi_rl
    disp(1:n) = addr(1:n)-base
    call mpi_type_create_struct( n,bl,disp,ty,tmptype,ierr )
  ...
  end subroutine arrivaltypeinit_tl
...
! declare parameters
!==============================================
  integer, parameter :: arrstorage = 1000
  integer, parameter :: nminarr = 10
...
!==============================================
  ...
  if (iallocstat /= 0) then
    write(unit=msgbuf,fmt='(2A)') 'ARR_MOD INITARR: ','Insufficient memory for TL matrix: reduce Nr*NRz'
  ...
  select case ( beam%runtype(1:1) )
  case ('A','a','e')
    nmaxarr = max(arrstorage/(nrz_per_range*pos%nrr),nminarr)
  case default
    nmaxarr = 1
  end select
  allocate( arr(nmaxarr,pos%nrr,nrz_per_range),stat=iallocstat )
  allocate( narrival(pos%nrr,nrz_per_range),stat=iallocstat )
  if (iallocstat /= 0) then
...
!==============================================
  ...
  do iz = 1, nrz
    do ir = 1, nrr
      ...
      write(unit=arrfile,fmt='(I0)') narrival(ir,iz)
      do iarr = 1, narrival(ir,iz)
        write(unit=arrfile,fmt=arrfmt) factor*arr(iarr,ir,iz)%a,rad2deg*arr(iarr,ir,iz)%phase,arr(iarr,ir,iz)%delayr,aimag(arr(iarr,ir,iz)%delay),&
&arr(iarr,ir,iz)%srcdeclangle,arr(iarr,ir,iz)%rcvrdeclangle,arr(iarr,ir,iz)%ntopbnc,arr(iarr,ir,iz)%nbotbnc
      end do
    end do
  end do
  99999 continue
...
!==============================================
  ...
  do iz = 1, nrz
    do ir = 1, nrr
      ...
      write(unit=arrfile,fmt='(I0)') narrival(ir,iz)
      do iarr = 1, narrival(ir,iz)
        write(unit=arrfile) factor*arr(iarr,ir,iz)%a,rad2deg*arr(iarr,ir,iz)%phase,arr(iarr,ir,iz)%delay,arr(iarr,ir,iz)%srcdeclangle,arr(iarr,ir,iz)&
&%rcvrdeclangle,real(arr(iarr,ir,iz)%ntopbnc),real(arr(iarr,ir,iz)%nbotbnc)
      end do
    end do
  end do
  99999 continue

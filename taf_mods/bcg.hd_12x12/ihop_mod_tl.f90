...
integer, private :: mpi_ihop_ray2d = mpi_datatype_null
integer, private :: mpi_ihop_ray2d_tl = mpi_datatype_null
integer, public :: nrz_per_range
logical, private :: ray2d_type_committed =  .false. 
logical, private :: ray2d_type_committed_tl =  .false. 
...
!----------------------------------------------
  if (mpi_ihop_ray2d_tl == mpi_datatype_null .or. &
      mpi_ihop_ray2d == mpi_datatype_null ) then
    call ray2dpttypeinit_tl( ray2d(1),ray2d_tl(1),ray2d(2),ray2d_tl(2) )
  endif
  call mpi_bcast( beam%nsteps,1,mpi_integer,root,comm,ierr )
  call mpi_bcast( ray2d_tl,nmax,mpi_ihop_ray2d_tl,root,comm,ierr )
  call mpi_bcast( ray2d,nmax,mpi_ihop_ray2d,root,comm,ierr )
...
!==============================================
  ...
  if (ray2d_type_committed_tl) then
    call mpi_type_free( mpi_ihop_ray2d_tl,ierr )
    mpi_ihop_ray2d_tl = mpi_datatype_null
    ray2d_type_committed_tl =  .false. 
  endif
...
!==============================================
  ...
  if ( .not. ray2d_type_committed) then
    ...
    extent = a2-base
    call mpi_type_create_resized( tmptype,0_mpi_address_kind,extent,mpi_ihop_ray2d,ierr )
  ...
  subroutine ray2dpttypeinit_tl( singleray2d, singleray2d_tl, nextray2d, nextray2d_tl )
!******************************************************************
!******************************************************************
!** This routine was generated by Automatic differentiation.     **
!** FastOpt: Transformation of Algorithm in Fortran, TAF 6.8.11  **
...
!----------------------------------------------
  ...
  if ( .not. ray2d_type_committed_tl) then
    bl = 1
    call mpi_get_address( singleray2d_tl,base,ierr )
    n = 1
    call mpi_get_address( singleray2d_tl%x_tl(1),addr(n),ierr )
    bl(n) = 2
    disp(n) = addr(n)-base
    ty(n) = mpi_rl
    n = n+1
    call mpi_get_address( singleray2d_tl%q_tl(1),addr(n),ierr )
    bl(n) = 2
    disp(n) = addr(n)-base
    ty(n) = mpi_rl
    n = n+1
    call mpi_get_address( singleray2d_tl%tau_tl,addr(n),ierr )
    disp(n) = addr(n)-base
    ty(n) = mpi_cl
    call mpi_type_create_struct( n,bl,disp,ty,tmptype,ierr )
    call mpi_get_address( nextray2d_tl,a2,ierr )
    extent = a2-base
    call mpi_type_create_resized( tmptype,0_mpi_address_kind,extent,mpi_ihop_ray2d_tl,ierr )
    call mpi_type_commit( mpi_ihop_ray2d_tl,ierr )
    call mpi_type_free( tmptype,ierr )
    ray2d_type_committed_tl =  .true. 
  endif

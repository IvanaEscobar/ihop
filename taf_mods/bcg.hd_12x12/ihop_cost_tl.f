...
C==============================================
      ...
      integer, parameter :: nobsmax_ihop = 750
...
C==============================================
      ...
      double precision :: myssp
      double precision :: myssp_tl
      integer :: num_file
...
C----------------------------------------------
            ...
            do bi = 1, nsx
              do iobs = 1, nobsmax_ihop
                if (iobs <= obsno_tiled(num_file,bi,bj)) then
                  ...
                  myssp_tl = ihop_ssp_tl(1,1,1,bi,bj)
                  myssp = ihop_ssp(1,1,1,bi,bj)
                  ihop_modvaltmp_tl = ihop_modvaltmp_tl+myssp_tl
                  ihop_modvaltmp = ihop_modvaltmp+myssp
                  ihop_modvaltmp_tl = ihop_modvaltmp_tl-myssp_tl
                  ihop_modvaltmp = ihop_modvaltmp-myssp
     ...
     $ihop_dummy(num_file,bi,bj) )
                  ...
                  ihopobs_buff_tl(irec) = ihop_modvaltmp_tl+ihopobs_buff_tl(irec)+myssp_tl
                  ihopobs_buff(irec) = ihopobs_buff(irec)+ihop_modvaltmp+myssp
                  ihopobs_buff_tl(irec) = ihopobs_buff_tl(irec)-myssp_tl
                  ihopobs_buff(irec) = ihopobs_buff(irec)-myssp
                endif
              end do
            end do
          end do
     ...
     $num_file) )
                call ihop_cost_read_obs( num_file,1,ihopobs_ind_glob(num_file,iobs),ihop_data,mythid )
                call ihop_cost_read_obs( num_file,-1,ihopobs_ind_glob(num_file,iobs),ihop_uncert,mythid )
                ...
                if (ihop_uncert > zerorl) then
                  ...
                  write(unit=standardmessageunit,fmt='(a,F16.6,a,i)') 'ESCOBAR, J=|m-d|^2(t)= ',objf_ihop(num_file),' Obs# ',
     $num_ihop(num_file)
                  write(unit=standardmessageunit,fmt='(a,F16.6,a,i)') 'ESCOBAR, "dJ/dT(t)= ',objf_ihop_tl(num_file),' Obs# ',
     $num_ihop(num_file)

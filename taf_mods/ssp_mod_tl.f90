...
!==============================================
  ...
  subroutine ccubic_tl( x, x_tl, c, c_tl, cimag, cimag_tl, gradc, gradc_tl, crr, crr_tl, crz, crz_tl, czz, czz_tl, rho, rho_tl, mythid )
!******************************************************************
!******************************************************************
!** This routine was generated by Automatic differentiation.     **
!** FastOpt: Transformation of Algorithm in Fortran, TAF 6.8.11  **
...
!==============================================
  ...
  subroutine clinear_tl( x, x_tl, c, c_tl, cimag, cimag_tl, gradc, gradc_tl, crr, crr_tl, crz, crz_tl, czz, czz_tl, rho, rho_tl, mythid )
!******************************************************************
!******************************************************************
!** This routine was generated by Automatic differentiation.     **
!** FastOpt: Transformation of Algorithm in Fortran, TAF 6.8.11  **
...
!==============================================
  ...
  subroutine cpchip_tl( x, x_tl, c, c_tl, cimag, cimag_tl, gradc, gradc_tl, crr, crr_tl, crz, crz_tl, czz, czz_tl, rho, rho_tl, mythid )
!******************************************************************
!******************************************************************
!** This routine was generated by Automatic differentiation.     **
!** FastOpt: Transformation of Algorithm in Fortran, TAF 6.8.11  **
...
! declare parameters
!==============================================
  integer, parameter :: ihop_max_nc_size = 15
  integer, parameter :: ihop_max_range = 6
  integer, parameter :: nrd = 1
  integer, parameter :: nrr = 1
  integer, parameter :: nsd = 1

!==============================================
! declare common blocks
!==============================================
  double precision :: ihop_alpha(2)
  double precision :: ihop_bcsound
  double precision :: ihop_bcsoundi
  double precision :: ihop_bcsoundshear
  double precision :: ihop_bcsoundsheari
  double precision :: ihop_brho
  double precision :: ihop_depth
  double precision :: ihop_dumpfreq
  double precision :: ihop_freq
  double precision :: ihop_idw_weights(ihop_max_range,ihop_max_nc_size)
  double precision :: ihop_ranges(ihop_max_range)
  double precision :: ihop_rd(nrd)
  double precision :: ihop_rr(nrr)
  double precision :: ihop_sd(nsd)
  double precision :: ihop_step
  double precision :: ihop_sumweights(ihop_max_range,ihop_max_nc_size)
  double precision :: ihop_xc(ihop_max_range,ihop_max_nc_size)
  double precision :: ihop_yc(ihop_max_range,ihop_max_nc_size)
  common /ihop_params_r/ ihop_dumpfreq, ihop_freq, ihop_depth, ihop_bcsound, ihop_bcsoundshear, ihop_brho, ihop_bcsoundi, ihop_bcsoundsheari, &
&ihop_sd, ihop_rd, ihop_rr, ihop_alpha, ihop_step, ihop_yc, ihop_xc, ihop_idw_weights, ihop_ranges, ihop_sumweights
...
!==============================================
  ...
  subroutine evalssp_tl( x, x_tl, c, c_tl, cimag, cimag_tl, gradc, gradc_tl, crr, crr_tl, crz, crz_tl, czz, czz_tl, rho, rho_tl, mythid )
!******************************************************************
!******************************************************************
!** This routine was generated by Automatic differentiation.     **
!** FastOpt: Transformation of Algorithm in Fortran, TAF 6.8.11  **
...
!==============================================
  ...
  real(kind=8), intent(inout) :: crr_tl
...
!----------------------------------------------
  select case ( grid%type )
  case ('N')
    call n2linear_tl( x,x_tl,c,c_tl,cimag,cimag_tl,gradc,gradc_tl,crr,crr_tl,crz,crz_tl,czz,czz_tl,rho,rho_tl,mythid )
  case ('C')
    call clinear_tl( x,x_tl,c,c_tl,cimag,cimag_tl,gradc,gradc_tl,crr,crr_tl,crz,crz_tl,czz,czz_tl,rho,rho_tl,mythid )
  case ('P')
    call cpchip_tl( x,x_tl,c,c_tl,cimag,cimag_tl,gradc,gradc_tl,crr,crr_tl,crz,crz_tl,czz,czz_tl,rho,rho_tl,mythid )
  case ('S')
    call ccubic_tl( x,x_tl,c,c_tl,cimag,cimag_tl,gradc,gradc_tl,crr,crr_tl,crz,crz_tl,czz,czz_tl,rho,rho_tl,mythid )
  case ('Q')
    call quad_tl( x,x_tl,c,c_tl,cimag,cimag_tl,gradc,gradc_tl,crr,crr_tl,crz,crz_tl,czz,czz_tl,rho,rho_tl,mythid )
  case default
    write(unit=msgbuf,fmt='(2A)') 'Profile option: ',grid%type
    if (ihop_dumpfreq >= 0) then
      call print_message( msgbuf,prtfile,squeeze_right,mythid )
...
!==============================================
  ...
  integer :: nznr_size
  real(kind=8) :: ssp1buffer
  real(kind=8), allocatable :: ssp3buffer(:,:,:)
  real(kind=8), allocatable :: sspglob(:)
  real(kind=8), allocatable :: ssptile(:,:,:,:)
  integer :: tkey
  ...
  if (allocated(ssptile)) then
    deallocate( ssptile )
  endif
  if (allocated(sspglob)) then
    deallocate( sspglob )
  endif
  if (allocated(ssp3buffer)) then
    deallocate( ssp3buffer )
  endif
  allocate( ssptile(grid%nz,grid%nr,nsx,nsy),stat=iallocstat )
  allocate( sspglob(nznr_size),stat=iallocstat )
  allocate( ssp3buffer(nsx,nsy,nznr_size),stat=iallocstat )
  if (iallocstat /= 0) then
    write(unit=msgbuf,fmt='(2A)') 'SSP_MOD::gcmSSP: ','Insufficient memory to store sspTile and/or sspGlob'
    call print_error( msgbuf,mythid )
...
do iz = 1, grid%nz-1
  ...
  if (iz >= 2) then
    if (ihop_sumweights(ii,iz-1) > 0 .and. ihop_idw_weights(ii,jj) == 0) then
      njj(ii) = ihop_npts_idw+1
    endif
  endif
  if (iz == 1) then
    ssp1buffer = ihop_ssp(i,j,iz,bi,bj)
    ssp1buffer = chen_millero(i,j,0,bi,bj,mythid)*ihop_idw_weights(ii,jj)/ihop_sumweights(ii,iz)
  else
    ssp1buffer = ihop_ssp(i,j,iz-1,bi,bj)
    if (ihop_sumweights(ii,iz-1) > 0.d0) then
      if (ihop_idw_weights(ii,jj) == 0.d0) then
        ssp1buffer = ssp1buffer-ssptile(iz,ii,bi,bj)
      else if (njj(ii) <= ihop_npts_idw) then
        ssp1buffer = ssp1buffer*ihop_idw_weights(ii,jj)/ihop_sumweights(ii,iz-1)
      else
        ssp1buffer = 0.d0
      endif
    endif
  ...
  if (iz >= 2) then
    if (iz == grid%nz-1 .or. ihop_sumweights(ii,iz-1) == 0.0) then
      ...
      if (njj(ii) >= ihop_npts_idw) then
        ...
        dcdz = (ssptile(k-1,ii,bi,bj)-ssptile(k-2,ii,bi,bj))/(grid%z(k-1)-grid%z(k-2))
        ssptile(k:grid%nz,ii,bi,bj) = ssptile(k-1,ii,bi,bj)+dcdz*grid%z(k:grid%nz)
...
!==============================================
  ...
  integer :: nznr_size
  real(kind=8) :: ssp1buffer
  real(kind=8) :: ssp1buffer_tl
  double precision :: ssp1bufferh
  double precision :: ssp1bufferh_tl
  real(kind=8), allocatable :: ssp3buffer(:,:,:)
  real(kind=8), allocatable :: ssp3buffer_tl(:,:,:)
  real(kind=8), allocatable :: sspglob(:)
  real(kind=8), allocatable :: sspglob_tl(:)
  real(kind=8), allocatable :: ssptile(:,:,:,:)
  real(kind=8), allocatable :: ssptile_tl(:,:,:,:)
  integer :: tkey
  real(kind=8) :: tolerance
...
!----------------------------------------------
  ...
  tolerance = 5d-5
  nznr_size = grid%nz*grid%nr
  if (allocated(ssptile_tl)) then
    deallocate( ssptile_tl )
  endif
  if (allocated(ssptile)) then
    deallocate( ssptile )
  endif
  if (allocated(sspglob_tl)) then
    deallocate( sspglob_tl )
  endif
  if (allocated(sspglob)) then
    deallocate( sspglob )
  endif
  if (allocated(ssp3buffer_tl)) then
    deallocate( ssp3buffer_tl )
  endif
  if (allocated(ssp3buffer)) then
    deallocate( ssp3buffer )
  endif
  allocate( ssptile_tl(grid%nz,grid%nr,nsx,nsy) )
  allocate( ssptile(grid%nz,grid%nr,nsx,nsy),stat=iallocstat )
  allocate( sspglob_tl(nznr_size) )
  allocate( sspglob(nznr_size),stat=iallocstat )
  allocate( ssp3buffer_tl(nsx,nsy,nznr_size) )
  allocate( ssp3buffer(nsx,nsy,nznr_size),stat=iallocstat )
  if (iallocstat /= 0) then
    write(unit=msgbuf,fmt='(2A)') 'SSP_MOD::gcmSSP: ','Insufficient memory to store sspTile and/or sspGlob'
    call print_error( msgbuf,mythid )
  ...
  ssptile_tl = 0._ikind6
  ssptile = 0.0d0
  sspglob_tl = 0._ikind5
  sspglob = 0.0d0
  ssp1buffer_tl = 0._ikind4
  ssp1buffer = 0.0d0
  ssp3buffer_tl = 0._ikind3
  ssp3buffer = 0.0d0
  do bj = mybylo(mythid), mybyhi(mythid)
    do bi = mybxlo(mythid), mybxhi(mythid)
      tkey = bi+(bj-1)*nsx+(ikey_dynamics-1)*nsx*nsy
...
do iz = 1, grid%nz-1
  ...
  if (iz == 1) then
    ssp1buffer_tl = ihop_ssp_tl(i,j,iz,bi,bj)
    ssp1buffer = ihop_ssp(i,j,iz,bi,bj)
    call chen_millero_tl( ssp1bufferh,ssp1bufferh_tl,i,j,0,bi,bj,mythid )
    ssp1buffer_tl = ssp1bufferh_tl*(ihop_idw_weights(ii,jj)/ihop_sumweights(ii,iz))
    ssp1buffer = ssp1bufferh*ihop_idw_weights(ii,jj)/ihop_sumweights(ii,iz)
  else
    ssp1buffer_tl = ihop_ssp_tl(i,j,iz-1,bi,bj)
    ssp1buffer = ihop_ssp(i,j,iz-1,bi,bj)
    if (ihop_sumweights(ii,iz-1) > 0.d0) then
      if (ihop_idw_weights(ii,jj) == 0.d0) then
        ssp1buffer_tl = ssp1buffer_tl-ssptile_tl(iz,ii,bi,bj)
        ssp1buffer = ssp1buffer-ssptile(iz,ii,bi,bj)
      else if (njj(ii) <= ihop_npts_idw) then
        ssp1buffer_tl = ssp1buffer_tl*(ihop_idw_weights(ii,jj)/ihop_sumweights(ii,iz-1))
        ssp1buffer = ssp1buffer*ihop_idw_weights(ii,jj)/ihop_sumweights(ii,iz-1)
      else
        ssp1buffer_tl = 0._ikind2
        ssp1buffer = 0.d0
      endif
    endif
  endif
  ssptile_tl(iz,ii,bi,bj) = ssp1buffer_tl+ssptile_tl(iz,ii,bi,bj)
  ssptile(iz,ii,bi,bj) = ssptile(iz,ii,bi,bj)+ssp1buffer
  if (iz >= 2) then
    if (iz == grid%nz-1 .or. ihop_sumweights(ii,iz-1) == 0.0) then
      k = iz
      if (njj(ii) >= ihop_npts_idw) then
        ...
        dcdz_tl = (-(ssptile_tl(k-2,ii,bi,bj)/(grid%z(k-1)-grid%z(k-2))))+ssptile_tl(k-1,ii,bi,bj)/(grid%z(k-1)-grid%z(k-2))
        dcdz = (ssptile(k-1,ii,bi,bj)-ssptile(k-2,ii,bi,bj))/(grid%z(k-1)-grid%z(k-2))
        ssptile_tl(k:grid%nz,ii,bi,bj) = dcdz_tl*grid%z(k:grid%nz)+ssptile_tl(k-1,ii,bi,bj)
        ssptile(k:grid%nz,ii,bi,bj) = ssptile(k-1,ii,bi,bj)+dcdz*grid%z(k:grid%nz)
        interp_finished =  .true. 
      else
        interp_finished =  .false. 
  ...
  do bj = mybylo(mythid), mybyhi(mythid)
    do bi = mybxlo(mythid), mybxhi(mythid)
      ...
      do jj = 1, grid%nr
        do ii = 1, grid%nz
          ssp3buffer_tl(bi,bj,k) = ssptile_tl(ii,jj,bi,bj)
          ssp3buffer(bi,bj,k) = ssptile(ii,jj,bi,bj)
          k = k+1
        end do
      end do
  ...
  call global_sum_vector_rl( nznr_size,ssp3buffer_tl,sspglob_tl,mythid )
  call global_sum_vector_rl( nznr_size,ssp3buffer,sspglob,mythid )
  k = 1
  do jj = 1, grid%nr
    do ii = 1, grid%nz
      ssp_tl%cmat_tl(ii,jj) = sspglob_tl(k)
      ssp%cmat(ii,jj) = sspglob(k)
      k = k+1
    end do
  end do
...
!==============================================
  ...
  subroutine n2linear_tl( x, x_tl, c, c_tl, cimag, cimag_tl, gradc, gradc_tl, crr, crr_tl, crz, crz_tl, czz, czz_tl, rho, rho_tl, mythid )
!******************************************************************
!******************************************************************
!** This routine was generated by Automatic differentiation.     **
!** FastOpt: Transformation of Algorithm in Fortran, TAF 6.8.11  **
...
!==============================================
  ...
  subroutine quad_tl( x, x_tl, c, c_tl, cimag, cimag_tl, gradc, gradc_tl, crr, crr_tl, crz, crz_tl, czz, czz_tl, rho, rho_tl, mythid )
!******************************************************************
!******************************************************************
!** This routine was generated by Automatic differentiation.     **
!** FastOpt: Transformation of Algorithm in Fortran, TAF 6.8.11  **

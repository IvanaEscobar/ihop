...
!==============================================
  integer :: ierr
  integer :: mpi_ihop_ray2d = mpi_datatype_null
  integer :: mpi_ihop_ray2d_tl = mpi_datatype_null
  logical :: ray2d_type_committed =  .false. 
  logical :: ray2d_type_committed_tl =  .false. 

!----------------------------------------------
! TANGENT LINEAR AND FUNCTION STATEMENTS
!----------------------------------------------
  if (mpi_ihop_ray2d == mpi_datatype_null) then
    call ray2dpttypeinit_tl( ray2d(1),ray2d_tl(1),ray2d(2),ray2d_tl(2),mpi_ihop_ray2d,&
      mpi_ihop_ray2d_tl,ray2d_type_committed,ray2d_type_committed_tl )
  endif
  call mpi_bcast( beam%nsteps,1,mpi_integer,root,comm,ierr )
...
!==============================================
  ...
  subroutine ray2dpttypeinit_tl( singleray2d, singleray2d_tl, nextray2d, nextray2d_tl, mytype, mytype_tl, type_committed, &
type_committed_tl )
!******************************************************************
!******************************************************************
!** This routine was generated by Automatic differentiation.     **
...
!----------------------------------------------
  if ( .not. type_committed) then
    ...
    ty(n) = mpi_cl
    call mpi_type_create_struct( n,bl,disp,ty,tmptype,ierr )
    call mpi_get_address( singleray2d_tl,a1,ierr )
    call mpi_get_address( nextray2d_tl,a2,ierr )
    extent = a2-a1
    call mpi_type_create_resized( tmptype,0_mpi_address_kind,extent,mytype_tl,ierr )
    call mpi_type_commit( mytype_tl,ierr )
    call mpi_type_free( tmptype,ierr )
    type_committed_tl =  .true. 
  endif

...
!  (used only for 'm')
  ...
  IF ( alphaT.GT.c ) THEN
    ...
    IF (BELLI_dumpfreq.LT.0) THEN
      ...
      WRITE( PRTFile, '(2A)' ) 'Usually this means you have '&
        'high attenuation'
    ENDIF

    WRITE(errorMessageUnit,'(2A)') 'ATTENMOD CRCI: complex sound '&
      'speed has an imaginary part > real part'

...
!$TAF store is,isegbot = TraceRay2D
          ...
          ELSEIF ( DistBegBot > 0.0d0 .AND. DistEndBot <= 0.0d0 ) THEN
            reflect=.true.
...
!$TAF store is,isegbot = TraceRay2D
          ...
          ELSE
            ! Do not reflect
            reflect=.false.
          ENDIF ! IF ( DistBegTop > 0.0d0 .AND. D
    ...
    ENDIF ! IF ( DistBegTop <= 0 .OR. DistBegBot <= 0 ) THEN

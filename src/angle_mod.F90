...
!EOP
  ...
  IF ( Angles%Nalpha.EQ.0 ) THEN   ! estimate Nalpha
    ...
    ELSE
      ...
      Angles%Nalpha = &
        MAX( INT( 0.3*Pos%RR( Pos%nRR )*BELLI_freq/c0 ), 300 )
      ...
      Angles%Nalpha = &
        MAX( INT( PI / d_theta_recommended ), Angles%Nalpha )
    ENDIF
  ...
  IF ( Angles%Nalpha.GT.1 &
     .AND. &
      ABS( MOD( Angles%adeg(Angles%Nalpha)-Angles%adeg(1), 360.0 ) ) &
     .LT.10.0*TINY(1.0D0) ) &
  ...
  IF ( Angles%Nalpha.GT.1 ) THEN
    Angles%Dalpha = &
      ( Angles%arad( Angles%Nalpha )-Angles%arad( 1 ) ) &
...
!EOP
  ...
  WRITE(msgBuf,'(2A)') 'ANGLEMOD ReadBearingElevationAngles:', &
    '3D rays not supported in belli'
  CALL PRINT_ERROR( msgBuf,myThid )
#endif /* BELLI_WRITE_OUT */
  ...
  IF ( Angles%Nbeta.EQ.0 ) THEN   ! automatically estimate Nbeta to use
    ...
    ELSE
      Angles%Nbeta = &
        MAX( INT( 0.1*Pos%RR( Pos%nRR )*BELLI_freq / c0 ), 300 )
    ENDIF
  ...
  IF ( Angles%Nbeta.GT.1 &
     .AND. & 
      ABS( MOD( &
            Angles%beta( Angles%Nbeta )-Angles%beta( 1 ), 360.0D0 ) )&
      .LT.10.0 * TINY( 1.0D0 ) ) &
  ...
  IF ( RunType( 6:6 ).EQ.'2' .AND. RunType( 1:1 ).NE.'R' ) THEN
    ...
    WRITE(msgBuf,'(2A)') 'Replacing beam take-off angles, beta, ' &
      'with receiver bearing lines, theta'
    ! In adjoint mode we do not write output besides on the first run
    IF (BELLI_dumpfreq.GE.0) &
    CALL PRINT_MESSAGE( msgbuf, PRTFile, SQUEEZE_RIGHT, myThid )
#endif /* BELLI_WRITE_OUT */
  ...
  IF (BELLI_dumpfreq.GE.0) THEN
    WRITE(msgBuf,'(A)')
    CALL PRINT_MESSAGE( msgbuf, PRTFile, SQUEEZE_RIGHT, myThid )
    WRITE(msgBuf,'(A,I10)') 'Number of beams in bearing   = ', &
     Angles%Nbeta
    CALL PRINT_MESSAGE( msgbuf, PRTFile, SQUEEZE_RIGHT, myThid )
  ...
  Angles%Dbeta = 0.0
  IF ( Angles%Nbeta.NE.1 ) &
    Angles%Dbeta = ( Angles%beta( Angles%NBeta ) - Angles%beta(1) ) &
                   / ( Angles%Nbeta - 1 )

...
!   Initiate fixed variable for belli time series. Note: NO BELLI_THREED here
  ...
  USE srpos_mod, only: Pos, ReadSXSY, ReadSZRz, &
...
!IESCO24: some notes
  ...
  CALL ReadRayElevationAngles( Depth, Bdry%Top%HS%Opt, Beam%RunType,&
         myThid )
#ifdef BELLI_THREED
...
!EOP
  ...
  CASE ( 'A' )                  ! *** Half-space properties ***
    ...
    HS%cp  = CRCI( zTemp, alphaR, alphaI, AttenUnit, bPower, fT, &
               myThid )
    HS%cs  = CRCI( zTemp, betaR,  betaI,  AttenUnit, bPower, fT, &
               myThid )

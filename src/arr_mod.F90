...
!EOP
  ...
  ELSE ! not a new ray
    ...
    Arr( Nt,ir,iz )%SrcDeclAngle  = w1 * &
                                      Arr( Nt,ir,iz )%SrcDeclAngle &
                                  + w2 * SrcDeclAngle
    Arr( Nt,ir,iz )%RcvrDeclAngle = w1 * &
                                      Arr( Nt,ir,iz )%RcvrDeclAngle &
                                  + w2 * RcvrDeclAngle
  ENDIF ! IF ( NewRay )
...
!EOP  
  ...
  CALL MPI_Bcast( geninfluence, arrSize, MPI_DOUBLE_PRECISION, root, &
                  comm, ierr )

...
! !USES:
  ...
  USE srpos_mod, only: WriteSxSy, WriteSzRz, WriteRcvrRanges, &
                       WriteFreqVec
...
! For MPI writing: inspo from eeboot_minimal.F
  ...
  IF ( .NOT.usingMPI ) THEN
    ...
    WRITE(fNam,'(4A)') TRIM(BELLI_fileroot),'.',myProcessStr(1:IL),&
      '.prt'
...
!EOP
  ...
  CASE ( 'F' )
    ...
    CALL PRINT_MESSAGE( msgbuf, PRTFile, SQUEEZE_RIGHT, myThid )
    WRITE( PRTFile, '(4(A,G11.4), A)')&
      ' T = ', T, 'degrees   S = ', Salinity, ' psu   pH = ', pH,&
      ' z_bar = ', z_bar, ' m'
  CASE ( 'B' )
    ...
    DO iBio = 1, NBioLayers
      ...
      WRITE(msgBuf, '(A,F10.4,A)') '      Top    of layer = ', &
        bio( iBio )%Z1, ' m'
      CALL PRINT_MESSAGE( msgbuf, PRTFile, SQUEEZE_RIGHT, myThid )
      WRITE(msgBuf, '(A,F10.4,A)') '      Bottom of layer = ', &
        bio( iBio )%Z2, ' m'
      CALL PRINT_MESSAGE( msgbuf, PRTFile, SQUEEZE_RIGHT, myThid )
      WRITE(msgBuf, '(A,F10.4,A)') '      Resonance frequency = ', &
        bio( iBio )%f0, ' Hz'
      CALL PRINT_MESSAGE( msgbuf, PRTFile, SQUEEZE_RIGHT, myThid )
...
!EOP
  ...
  CASE ( 'S' )
    ...
    CALL PRINT_MESSAGE( msgbuf, PRTFile, SQUEEZE_RIGHT, myThid )
  CASE ( 'b' )
    WRITE(msgBuf,'(2A)') 'Geometric gaussian beams in '&
      'ray-centered coordinates'
    CALL PRINT_MESSAGE( msgbuf, PRTFile, SQUEEZE_RIGHT, myThid )
  CASE ( 'B' )
    WRITE(msgBuf,'(A)') 'Geometric gaussian beams in '&
      'Cartesian coordinates'
    CALL PRINT_MESSAGE( msgbuf, PRTFile, SQUEEZE_RIGHT, myThid )
  CASE ( 'g' )
    WRITE(msgBuf,'(A)') 'Geometric hat beams in '&
      'ray-centered coordinates'
    CALL PRINT_MESSAGE( msgbuf, PRTFile, SQUEEZE_RIGHT, myThid )
  CASE DEFAULT
  END SELECT
...
!EOP
  ...
  ELSE   ! compressed format for TL from FIELD3D
    ...
    LRecl = MAX( 41, 2*Nfreq, Pos%nTheta, Pos%nSZ, Pos%nRZ, &
              2*Pos%nRR )

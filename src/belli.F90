...
!
! MITgcm version (2026) writting by Ivana Escobar at Oden Institute
!
        ...
        !!IESCO22: end BEAM stuff !!
          ...
          IF ( MOD( iAlpha-1, MAX( Angles%nAlpha/50, 1 ) ).EQ.0 ) THEN
            ...
            IF (BELLI_dumpfreq.GE.0) THEN
              CALL PRINT_MESSAGE(msgBuf, PRTFile, SQUEEZE_RIGHT, &
                     myThid)
              FLUSH( PRTFile )
            ENDIF
...
! reflect :: Logical flag for ray reflection
  ...
  REAL (KIND=_RL90) :: dEndTop(2), dEndBot(2), &
                       TopnInt(2), BotnInt(2), &
...
!$TAF store beam%runtype = TraceRay2D
  IF ( Beam%RunType( 2:2 ).EQ.'G' .OR. &
       Beam%RunType( 2:2 ).EQ.'B' )  THEN
...
#ifdef BELLI_WRITE_OUT
    WRITE(msgBuf,'(A)') &
      'WARNING: TraceRay2D: Source is outside domain boundaries'
    ! In adjoint mode we do not write output besides on the first run
    IF (BELLI_dumpfreq.GE.0) &
      CALL PRINT_MESSAGE(msgBuf, PRTFile, SQUEEZE_RIGHT, myThid)
#endif /* BELLI_WRITE_OUT */
...
!$TAF store ray2d = TraceRay2D
        ...
        IF ( iH.GT.1 ) THEN
          ...
          RayTurn = ( declAlpha.LE.0. .AND. declAlphaOld.GT.0. .OR. &
                      declAlpha.GE.0. .AND. declAlphaOld.LT.0. )
...
! !ROUTINE: Reflect2D
! !INTERFACE:
  SUBROUTINE Reflect2D( iH, HS, BotTop, tBdry, nBdry, kappa, &
                        RefC, Npts, myThid )
...
! RInt :: Local derived type for reflection coefficient
  ...
  REAL (KIND=_RL90)    :: RM, RN, Tg, Th, rayt( 2 ), rayn( 2 ), &
                          ...
                          csjump
  REAL (KIND=_RL90)    :: ck, co, si, cco, ssi, pdelta, &
                          rddelta, sddelta, theta_bot
  COMPLEX (KIND=_RL90) :: kx, kz, kzP, kzS, kzP2, kzS2, mu, f, g, &
                          y2, y4, Refl
...
!$TAF store ray2D(iH)%t = reflect2d1
  ...
  ! Get c
  CALL evalSSP( ray2D( iH )%x, c, cimag, gradc, crr, crz, czz, rho, &
                myThid )
